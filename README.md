...
씬에 띄울 현재 카드 선택창 제작과 덱 시스템 연동 - 2022-4-09 70% 완성
...
0406 ~ 0413 
윤정민:체력 데미지 폰트 구현(일단은 숫자와 월드포지션만 받아오면 화면에 띄워주는 기능 구현):完 , 씬트리 구현:10%, 카드 선택 화면과 선택한 덱의 인게임과 연동 구현):70%, 적 플레이어AI의 덱 시스템 구현 
